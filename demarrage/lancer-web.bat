...
title Agentic - demarrage web

rem =============================================================================
rem  Demarre toute la chaine et ouvre le front dans le navigateur.
rem
rem    lancer-web.bat            mode demo (defaut)
rem    lancer-web.bat demo       modele simule + API locale + front
rem    lancer-web.bat reel       API locale (config.toml du depot) + front,
...
rem     section [api] de config.toml, et le port du front doit figurer dans
rem     [api] cors_origins (1420 y est par defaut).
...
set "TITLE_FRONT=Agentic - front navigateur"
...
echo Usage : lancer-web.bat [demo^|reel]
...
rem -----------------------------------------------------------------------------
rem  7. Le front, branche sur l'API
rem -----------------------------------------------------------------------------
echo --- Front ^(navigateur^) ---
cd /d "%FRONT_DIR%"
set "VITE_API_BASE_URL=%API_BASE%"
echo VITE_API_BASE_URL=%VITE_API_BASE_URL%
rem Le port 1420 est celui que vite.config.ts epingle (strictPort) et il figure
rem dans les origines CORS livrees par le backend : ne pas en changer sans
rem ajouter la nouvelle origine a [api] cors_origins.
start "%TITLE_FRONT%" cmd /k "npm run dev -- --port %FRONT_PORT% --strictPort"
echo Attente que le serveur de developpement reponde sur %FRONT_URL% ...
call :wait_url "%FRONT_URL%" 40
if errorlevel 1 (
  echo [ERREUR] Le front n'a pas repondu sur %FRONT_URL%.
  echo Regardez la fenetre "%TITLE_FRONT%" : le port %FRONT_PORT% est peut-etre deja pris.
  goto :fail
)
echo Front pret.
start "" "%FRONT_URL%"
...
echo  Fenetre "%TITLE_FRONT%"  le serveur de developpement du front, sur %FRONT_URL%
echo  Navigateur                      l'application, ouverte sur %FRONT_URL%
echo.
...
echo.
echo  Pour tout arreter : fermez chaque fenetre ci-dessus ^(ou Ctrl+C dedans^),
echo  puis l'onglet du navigateur. Cette fenetre-ci peut etre fermee des maintenant.
echo =============================================================================
echo.
pause
endlocal
