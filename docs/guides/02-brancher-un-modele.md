...
Une réponse que le codec ne sait pas lire est une `MODEL_PROTOCOL_ERROR / UNPARSEABLE_REPLY`, jamais rejouée — rejouer le même appel ne changerait rien —, dont l'enregistrement d'échec garde le nom du codec, l'index de l'élément, un **extrait de la forme brute** (`excerpt`, 500 caractères) et une `reason` (`no_json_found`, `json_unbalanced`, `json_invalid`, `path_not_found`, `unexpected_type`, `missing_conversation_id`, `unexpected_tool`, `no_envelope`). Elle ne termine pas la session pour autant : depuis [ADR-023](../adr/ADR-023-politique-de-correction.md), elle est d'abord **persistée** telle qu'on a pu la citer (un message entrant invalide portant `{"raw": <extrait>, "reason": …}`), puis l'application renvoie au modèle un `protocol_correction_request` — la faute exacte, les types attendus à cet instant, un rappel de leur forme, un exemple minimal valide — et **relit**. Au plus `protocol.max_correction_attempts` réponses inutilisables d'affilée sont tolérées (5 par défaut, `[protocol]`) ; la moindre réponse valide remet le compteur à zéro, et une correction ne consomme ni cycle ni plan. Ce n'est qu'ensuite que l'ancienne conduite reprend : si la fenêtre de contexte n'est pas `HEALTHY` et que `context.rotate_on_unusable_reply_in_warning` est vrai (défaut), l'application **rotationne** (nouvelle conversation, résumé) — une fenêtre déjà saturée rotationne d'ailleurs sans corriger, puisqu'une correction n'y tiendrait pas —, sinon la session échoue ([ADR-019](../adr/ADR-019-consolidation-vague-1.md)). `max_correction_attempts = 0` rétablit le comportement d'avant : la première réponse inutilisable termine la session.
...
Avant la table : une erreur de protocole n'est plus fatale sur-le-champ (ADR-023). Face à un modèle qui répond mal, l'opérateur voit d'abord passer des événements `correction.requested` et des messages sortants `protocol_correction_request` — la CLI affiche alors une ligne `Correction` (« waiting for a corrected reply », avec le rang de la tentative sur la borne) et l'API les liste sur `GET /sessions/{sid}/corrections` —, et ce n'est qu'après `protocol.max_correction_attempts` réponses inutilisables d'affilée que la session rotationne ou échoue. Les `details` de l'échec final disent combien de corrections ont été tentées (`corrections_attempted`, `unusable_replies`, `max_correction_attempts`) : un modèle fautif une fois et un modèle incapable de se corriger cinq fois de suite ne se dépannent pas de la même façon.
...
| `UNPARSEABLE_REPLY` (`codec`, `reason`, `excerpt`) | le codec n'a pas trouvé de message dans la réponse brute ; la réponse est persistée et le modèle est corrigé avant tout échec | lire `excerpt` : mauvais `content_path` ? prose sans JSON ? mauvais `outbound` (le modèle a reçu un objet là où il attendait du texte) ? un modèle qui ne corrige jamais trahit plutôt un codec mal choisi qu'un modèle capricieux |
| `SCHEMA_INVALID` sur un `message.rejected` | l'enveloppe est lue mais ne respecte pas le schéma (champ manquant, `type` inattendu, `conversation_id` absent) | le modèle ne suit pas les instructions du protocole ; voir l'événement pour le champ fautif — et le `protocol_correction_request` qui suit, qui contient exactement ce qui a été reproché au modèle |
| Beaucoup de `correction.requested`, aucune réponse valide | le modèle reçoit bien les corrections mais ne les applique pas (ou la correction ne lui parvient pas : `outbound` du codec, gabarit de `post`) | comparer `corrections_total{error_code}` à `messages_rejected_total` ; lire une correction sur `GET /sessions/{sid}/corrections` (`reminder`, `example`, `expected_types`) et vérifier qu'elle est bien transmise telle quelle |
| Session `FAILED` avec `corrections_attempted = max_correction_attempts` | le modèle n'a pas su se corriger dans la borne | augmenter `protocol.max_correction_attempts`, ou revoir ce que le modèle reçoit (instructions du protocole, codec) ; `0` rétablit l'échec immédiat |
