...
| `model_interaction` | par session, alimenté par `message.outbound` / `message.retransmitted` / `message.inbound` / `message.rejected` / `correction.requested` (contrat §4) ; `rebuild()` le **conserve** (les statuts HTTP ne sont pas persistés) ; un tracker à froid le reconstruit depuis les `MessageRecord` de la conversation courante (types et `validation_status`, statuts HTTP `None`, correction en vol déduite du dernier sortant). `correction_attempt` / `correction_max_attempts` sont remis à zéro dès qu'un message valide arrive (ADR-023). | §4.1, ADR-023 |
...
| `correction.requested` | phase 9 (ADR-023) | + `cycle_id` (celui du message corrigé) | `{"message_id": str, "error_code": str, "attempt": int, "max_attempts": int, "expected_types": [str], "rejected_message_id": str \| null}` | audit, tracker (`correction_attempt` / `correction_max_attempts` tant qu'une correction est en vol), télémétrie (`corrections_total{error_code}`) |
...
| `user_response.received` | phase 9 (ADR-022) | + `cycle_id` | `{"message_id": str, "format": str, "status": str, "expects_reply": bool, "body_bytes": int, "auto_close_skipped": bool, …}` — jamais le corps | audit, tracker (relit session et conversation, comme tout événement) |
...
| `corrections_total{error_code}` | counter | `correction.requested` (`error_code` du refus, `unknown` à défaut) — ADR-023 |
