...
flowchart TD
    ...
    G2 --> R
    G1 -- "fail : MODEL_GET_TIMEOUT épuisé en fenêtre non HEALTHY" --> G3["Fenêtre SATURATED (unusable_reply)"]
    G3 --> R
    G1 -- "fail : UNPARSEABLE_REPLY (codec)" --> P2b["MessageRecord invalid (raw, reason), protocol_error_count + 1 ; message.rejected ; FailureRecord"]
    P2b --> P3
    G1 -- "fail (autre)" --> F
    ...
    P2 --> P3{"ADR-023 : max_correction_attempts > 0, budget de corrections non épuisé et fenêtre non SATURATED ?"}
    P3 -- oui --> P3B["POST protocol_correction_request (ni cycle, ni plan) ; correction.requested ; relecture contre la même attente"]
    P3B --> G
    P3 -- non --> P3C{"Fenêtre non HEALTHY et rotate_on_unusable_reply_in_warning ?"}
    P3C -- oui --> G3
    P3C -- non --> F
    P1 -- oui --> P4["MessageRecord valid, get_cursor, context_bytes ; message.inbound ; evaluate — compteur de corrections remis à zéro"]
    P4 --> T{"Type ?"}
...
- **Un message rejeté est persisté** avec `validation_status = invalid` sous son `message_id` (un identifiant généré s'il est absent ou déjà connu), son type si c'est un `MessageType` connu, sinon `system_error` ; sa taille compte dans `context_bytes` et le curseur avance (ADR-013 : il est bien dans le contexte du modèle). Une réponse que le **codec** n'a pas su lire laisse la même trace (`{"raw": <extrait>, "reason": …}` sous `system_error`, ADR-021 §2 amendé par ADR-023), quel que soit le réglage de `max_correction_attempts` : c'est une règle de persistance, pas une politique.
- **Le compteur de corrections est dérivé, jamais persisté** : `_unusable_replies` vit en mémoire et `consecutive_unusable_replies(store, conversation)` le reconstruit depuis les enregistrements entrants invalides de fin de trace quand une session reprend après un crash — le redémarrage n'est pas un moyen de rendre la borne d'ADR-023 inopérante. Symétriquement, `pending_outbound_of` traverse les `protocol_correction_request` : `M` reste « en attente » et une session interrompue en pleine correction se reprend comme une session interrompue avant la première réponse (ADR-016).
- **Rien n'est envoyé au modèle** sur `BUDGET_EXCEEDED`, `ROTATION_FAILED` ou interruption ; une réponse inutilisable fait exception depuis ADR-023 — c'est précisément le `protocol_correction_request` qui part avant tout échec —, mais l'échec lui-même, lui, n'envoie toujours rien. La conversation distante est fermée en best effort à l'échec (ADR-006, ADR-012), jamais à l'interruption (c'est l'`InterruptionHandler` qui s'en charge).
...
| `fail` | erreur de protocole (`parse_inbound` ou `UNPARSEABLE_REPLY`), politique de correction active, budget non épuisé, fenêtre non `SATURATED` (ADR-023) | `message.rejected`, `FailureRecord`, puis `protocol_correction_request` POSTé (`correction.requested`) et relecture contre la **même** attente : ni cycle, ni plan, `M` reste le message en attente |
| `fail` | réponse inutilisable (erreur de protocole après le budget de corrections, ou `MODEL_GET_TIMEOUT` épuisé) **et** fenêtre non `HEALTHY` sous `rotate_on_unusable_reply_in_warning` (ADR-019 §2) | fenêtre `SATURATED` (raison `unusable_reply`), rotation avec `M`, budget de corrections neuf dans l'enfant |
| `fail` | tout autre cas (`max_attempts_exhausted`, `circuit_open`, `non_retryable:<type>`, réponse inutilisable sans correction ni rotation possibles) | cycle `FAILED`, conversation `FAILED` (raison `failure`), session `FAILED`, close distant best effort ; les `details` de la dernière erreur portent `unusable_replies` / `corrections_attempted` / `max_correction_attempts` quand la politique est active |
...
| `correction.requested` | orchestrateur (ADR-023) | `cycle_id` (celui du message corrigé) | `{message_id, error_code, attempt, max_attempts, expected_types, rejected_message_id?}` — jamais le rappel ni l'exemple, relus dans la table des messages |
...
| erreur de protocole, budget de corrections restant | inchangé (aucun cycle ouvert) | `WAITING_MODEL_RESPONSE`, `protocol_error_count + 1`, curseur avancé | `RUNNING` | `MessageRecord` entrant `invalid`, `message.rejected`, `FailureRecord MODEL_PROTOCOL_ERROR`, `MessageRecord` sortant `protocol_correction_request` + `message.outbound` + `correction.requested` (ADR-023) |
| erreur de protocole en `HEALTHY`, corrections épuisées (ou `max_correction_attempts = 0`) | `FAILED` | `→ FAILED`, `protocol_error_count + 1`, curseur avancé | `FAILED` (`failure`) | comme ci-dessus, sans correction : la dernière erreur porte `corrections_attempted` |
| réponse inutilisable en fenêtre non `HEALTHY`, corrections épuisées | inchangé, continue dans l'enfant | parent `ROTATING → CLOSED` si la rotation aboutit | `RUNNING` | comme une rotation |
...
4. Rien n'est envoyé au modèle sur interruption, budget dépassé ou rotation échouée ; une erreur de protocole, elle, donne lieu à un `protocol_correction_request` tant que le budget d'ADR-023 le permet, et à rien du tout ensuite. Une interruption n'écrit rien depuis l'orchestrateur — et le jeton est vérifié avant de composer une correction comme avant toute autre étape.
...
| Politique d'avant ADR-023, épinglée avec `protocol.max_correction_attempts = 0` (la boucle de correction a ses propres tests, `tests/integration/test_phase9_correction.py`) : erreur de protocole en `HEALTHY` ⇒ `FAILED` (`message.rejected`, `FailureRecord`, curseur, snapshot) ; en `WARNING` ⇒ rotation avec le suivi retransmis (ADR-019 §2) ; `MODEL_GET_TIMEOUT` épuisé en `WARNING` ⇒ rotation | `given_unexpected_message_type_in_healthy_window_when_received_then_session_failed_with_protocol_error`, `given_unexpected_message_in_warning_window_when_received_then_rotation_and_follow_up_retransmitted`, `given_get_timeout_exhausted_in_warning_window_when_decided_fail_then_rotation_instead` |
