...
Les agents implémentent exactement ces surfaces (les paramètres optionnels peuvent être ajoutés).

| Classe | Méthodes publiques |
|---|---|
| `ConversationLifecycleManager(store, bus, clock, ids)` | `create_session(goal, user_message, user_id, budget, auto_close) -> SessionRecord` · `transition_session(session_id, to, *, reason=None) -> SessionRecord` · `create_conversation(session_id, *, parent_conversation_id=None, context_window_state=HEALTHY) -> ConversationRecord` · `transition_conversation(conversation_id, to, *, reason=None, **updates) -> ConversationRecord` · `update_conversation(conversation_id, **updates) -> ConversationRecord` · `update_session(session_id, **updates) -> SessionRecord` · `interrupt_conversation(conversation_id, *, reason) -> ConversationRecord` · `get_session / get_conversation` |
| `ProtocolAdapter(config)` | `build_user_request(...)`, `build_execution_result(...)`, `build_context_resume_request(...)` → `OutboundMessage` · `parse_inbound(raw: dict, *, expected: frozenset[MessageType], conversation, known_plan_ids, known_task_ids, stored_task_ids) -> InboundMessage` · `expected_inbound(last_outbound: MessageRecord | None, conversation: ConversationRecord) -> frozenset[MessageType]` |
...
| `HttpTransportGateway(config.transport, clock)` | `async init_conversation(instructions, metadata) -> str` · `async post_message(remote_conversation_id, payload) -> PostAck` · `async get_messages(remote_conversation_id, after) -> GetResult` · `async close_conversation(remote_conversation_id)` · `abandon()` |
| `FailureManager(config, store, bus, clock, ids)` | `classify(exc) -> NormalizedError` · `decide(error, attempt, context) -> Decision(retry|abort|rotate|fail, delay_ms)` · `record(error, ...) -> FailureRecord` |
...
| `AuditLog(store, clock, ids)` | `handle(event)` (abonné critique) · `verify(session_id) -> AuditVerification` |
| `ExecutionTracker(store)` | `handle(event)` · `snapshot(session_id) -> RuntimeSnapshot` |
| `TelemetryService()` | `handle(event)` · `render_text() -> str` · `metrics() -> dict` |
