...
│   │                              NOT_RUN_EXIT_CODES, command_not_run_reason : 126/127, 9009   (ADR-032)
...
| `PlanRunner(store, bus, executor, payload_guard, clock, ids, config, *, failure_manager=None, result_collector=None, scratch=None, translator=None)` | `async run(plan: PlanRecord, tasks: list[TaskRecord], session: SessionRecord, *, interrupt: CancellationToken) -> PlanOutcome(plan, tasks, execution_result \| None, interrupted, budget_exceeded, stop_reason)` · propriété `shell_dialect` (le dialecte du traducteur, qui dit quels codes sont la réponse du shell, ADR-032) |
