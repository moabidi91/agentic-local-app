...
# 8. le contrat que reçoit le modèle à chaque conversation (ADR-031)
uv run agentic-app protocol show              # le texte rendu pour cette machine, à l'octet près
uv run agentic-app protocol show --dialect powershell --json   # un autre dialecte, taille et part du budget
```
...
**Le contrat du modèle** ([ADR-031](docs/adr/ADR-031-contrat-du-modele-regles-dictionnaire-exemples.md)). Le texte envoyé au modèle à l'ouverture de chaque conversation est tout le contrat entre lui et l'application. Il s'ouvre sur dix règles non négociables (une enveloppe JSON et rien d'autre, noms et valeurs exacts, aucun champ inventé, types JSON exacts, identifiants neufs, type permis à cet instant…) et se ferme sur les mêmes, en liste de contrôle ; entre les deux, un dictionnaire de chaque champ, un échange complet découverte comprise, les erreurs courantes et leur forme juste. Les commandes des exemples sont écrites dans le dialecte que la machine annonce. Les exemples ne peuvent pas dériver du code : un test les rejoue contre l'adaptateur, le collecteur de résultats, le réducteur de contexte et la correction réels, un autre compare chaque table aux modèles pydantic. La frontière est stricte sur les types : `"2048"` n'est plus lu comme un entier, ni `"yes"` comme un booléen. Le texte rendu pèse environ 59 Ko, soit 15 % du budget de contexte par défaut.

**Un programme que le shell n'a pas pu lancer n'est jamais un verdict** ([ADR-032](docs/adr/ADR-032-programme-introuvable-jamais-un-verdict.md)). Toute commande passe par le shell, et quand le programme qu'elle nomme n'existe pas, c'est le shell qui répond : il démarre, écrit « command not found » et sort en **127** (**126** pour un fichier trouvé mais non exécutable). La boucle de compilation réelle a montré ce que cela donnait pour un programme reconnu : `/opt/no-such-jdk/bin/javac Main.java`, ou `mvn` sur une machine sans Maven, revenait avec `failure_is_verdict: true` et le plan continuait — l'application demandait au modèle d'analyser la réponse d'un compilateur qui n'avait jamais tourné. **Un verdict exige désormais la preuve que le programme a tourné** : 126 et 127 ne sont jamais un verdict, le plan suit la règle de tout échec (il s'arrête, sauf `continue_on_error`), et le résultat le dit — `execution` reste `ran`, puisque le shell a bien tourné, `reason` vaut `COMMAND_NOT_FOUND` ou `COMMAND_NOT_EXECUTABLE`, dérivé du code sans aucune colonne, et stderr porte le message du shell. C'est la convention du shell, donc une présomption quand un programme sort en 127 de lui-même : elle ne rend l'application que plus prudente, jamais moins. PowerShell, qui sortait en 1 comme une compilation ratée, reçoit un script qui rend 127 et 126 (non vérifié sur une vraie machine Windows : sa construction l'est) ; `cmd /c` garde son lancement et son code 9009, lu comme « introuvable ». Le contrat du modèle, qui rangeait le programme introuvable sous `not_started`, est corrigé et illustré par un exemple ; `rustc` rejoint `cargo` dans la liste par défaut.
